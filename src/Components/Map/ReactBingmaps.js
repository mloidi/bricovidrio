  ...
  componentWillUnmount() {
    try {
      let mapReference = this.props.id
        ? '#' + this.props.id
        : '.react-bingmaps';
      if (map[mapReference]) map[mapReference] = undefined;
      infobox = {};
      pendingProps = [];
    } catch (exception) {
    ...
    if (map[mapReference] && pushPins) {
      ...
      for (
        ...
        if (
          ...
          let option = pushPins[pushPinIndex].option
      ...
      if (infoboxes) {
        for (
          ...
          if (
            ...
            let option = infoboxes[infoboxIndex]
              ? infoboxes[infoboxIndex].option
              : null;
            if (option.htmlContent) {
              option.htmlContent = ReactDOMServer.renderToStaticMarkup(
                option.htmlContent
              );
            }
